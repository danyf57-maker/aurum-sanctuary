 ...
 */
async function callDeepSeekForInsight(derivedMemoryLite: any): Promise<string> {
 ...
 */
function buildUserPrompt(lite: any): string {
 ...
 * Runs every Sunday at 9am UTC
 */
export const generateInsight = onSchedule(
    {
        ...
        try {
            ...
            for (const userDoc of usersSnapshot.docs) {
                ...
                try {
                    ...
                    if (!createdAt) {
                        console.log(`⏭️  Skipping user ${uid}: No createdAt`);
                        skipCount++;
                        continue;
                    }
                    ...
                    if (daysSinceSignup < 7) {
                        console.log(`⏭️  Skipping user ${uid}: Only ${daysSinceSignup} days since signup`);
                        skipCount++;
                        continue;
                    }
                    ...
                    if (!liteDoc.exists) {
                        console.log(`⏭️  Skipping user ${uid}: No DerivedMemoryLite`);
                        skipCount++;
                        continue;
                    }
                    ...
                    if (!lite || (lite.totalEntries || 0) < 3) {
                        console.log(`⏭️  Skipping user ${uid}: Only ${lite?.totalEntries || 0} entries`);
                        skipCount++;
                        continue;
                    }

                    // Generate insight with DeepSeek
                    console.log(`🤖 Generating insight for user ${uid}...`);
                    ...
                    console.log(`✅ Insight generated for user ${uid}`);
                    successCount++;
                } catch (userError) {
                    console.error(`❌ Error generating insight for user ${uid}:`, userError);
                    errorCount++;
                }
            }
            ...
            console.error('❌ Fatal error in insight generation:', error);
            throw error;
        }
    }
);
