...
export function buildStrictReplyLanguageInstruction(
  ...
  const instructions: Record<ReflectionLanguage, { exact: (name: string) => string; sameAsUser: string }> = {
    en: {
      exact: (name) =>
        `Language rule (strict): Your final answer must be entirely in ${name}. The user's message language takes priority over the app locale. App locale: ${localeLabel}. Never mention the language the user wrote in. Do not open with a greeting unless the user greeted you first in the same turn. Respond directly to the meaning and emotional content. Prefer idiomatic, natural phrasing in the target language. If you are unsure, choose simpler native wording over literal translation.`,
      sameAsUser:
        `Language rule (strict): Reply in the same language as the user's message, even if the app locale is different. Supported reflection languages: English, French, Spanish, Italian, German, Portuguese. If the language is ambiguous, fall back to the app locale. App locale: ${localeLabel}. Never mention the user's language explicitly. Do not open with a greeting unless the user greeted you first in the same turn. Prefer idiomatic, natural phrasing over literal translation.`,
    },
    fr: {
      exact: (name) =>
    ...
    de: {
      ...
      sameAsUser:
        `Sprachregel (streng): Antworte in derselben Sprache wie die Nachricht der Person, auch wenn die App in einer anderen Sprache ist. Unterstützte Reflexionssprachen: Englisch, Französisch, Spanisch, Italienisch, Deutsch und Portugiesisch. Wenn die Sprache unklar bleibt, nimm die App-Sprache als Rückfall. App-Sprache: ${localeLabel}. Sprich die erkannte Sprache niemals an. Beginne nicht mit einer Begrüßung, es sei denn, die Person hat gerade zuerst gegrüßt. Bevorzuge idiomatische Formulierungen statt wörtlicher Übersetzung.`,
    },
    pt: {
      exact: (name) =>
        `Regra de língua (estrita): a tua resposta final deve estar inteiramente em ${name}. A língua da última mensagem da pessoa tem prioridade sobre a língua da aplicação. Língua da aplicação: ${localeLabel}. Nunca menciones a língua usada pela pessoa. Não abras com uma saudação, a menos que a pessoa tenha saudado neste mesmo turno. Responde diretamente ao sentido e ao vivido emocional. Prefere formulações idiomáticas e naturais. Se houver dúvida, escolhe um português mais simples em vez de uma tradução literal.`,
      sameAsUser:
