...
export default function MagazinePage() {
  ...
  const fetchIssuesPage = useCallback(
    async ({ reset }: { reset: boolean }) => {
      ...
      try {
        ...
        const validDocs = snap.docs.filter((d) => d.data()?.createdAt != null);
        lastDocRef.current = validDocs.length > 0 ? validDocs[validDocs.length - 1] : null;
  ...
  const toggleEntryInCollection = async (collectionId: string, entryId: string) => {
    ...
    if (!target) return;
    const alreadyIn = target.entryIds.includes(entryId);

    await updateDoc(doc(db, 'users', user.uid, 'collections', collectionId), {
      entryIds: alreadyIn ? arrayRemove(entryId) : arrayUnion(entryId),
      updatedAt: serverTimestamp(),
    });
      ...
      if (!response.ok) {
        ...
        for (const docSnap of entriesSnap.docs) {
          ...
          if (isEncrypted && data.createdAt) {
            ...
            dateTitle = `Réflexion du ${entryDate.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}`;
          }
          ...
          {filteredIssues.length === 0 ? (
            ...
            >
              {filteredIssues.map((issue) => {
                ...
                const issueCollections = collections.filter((c) => c.entryIds.includes(issue.id));
                            ...
                            {issueCollections.map((col) => (
                              <button
                                ...
                                onClick={(event) => {
                                  event.preventDefault();
                                  event.stopPropagation();
                                  void toggleEntryInCollection(col.id, issue.id);
                                }}
